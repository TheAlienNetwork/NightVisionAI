...
class EvidenceManager:
    ...
    def get_file_analysis_results(self, file_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if USE_SQLITE:
                # Get AI analysis results
                ai_query = "SELECT * FROM ai_analysis WHERE file_id = ?"
                ai_results = execute_query(ai_query, (file_id,), fetch=True) or []

                # Get facial recognition results
                face_query = "SELECT * FROM facial_recognition WHERE file_id = ?"
                face_results = execute_query(face_query, (file_id,), fetch=True) or []

                # Get forensics results
                forensics_query = "SELECT * FROM forensics_results WHERE file_id = ?"
                forensics_results = execute_query(forensics_query, (file_id,), fetch=True) or []
            else:
                # Get AI analysis results
                ai_query = "SELECT * FROM ai_analysis WHERE file_id = %s"
                ai_results = execute_query(ai_query, (file_id,), fetch=True) or []

                # Get facial recognition results
                face_query = "SELECT * FROM facial_recognition WHERE file_id = %s"
                face_results = execute_query(face_query, (file_id,), fetch=True) or []

                # Get forensics results
                forensics_query = "SELECT * FROM forensics_results WHERE file_id = %s"
                forensics_results = execute_query(forensics_query, (file_id,), fetch=True) or []
            ...
            st.error(f"Error retrieving file analysis results: {str(e)}")
            return {}